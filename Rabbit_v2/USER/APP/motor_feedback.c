...
void motor_speed_pid_cal_callback(MOTOR *_motor)
{
    PID_calc(&_motor->pid_param_speed, _motor->feedback.speed_rpm, _motor->target_rpm);
}


void motor_angle_pid_cal_callback(MOTOR *_motor)
{
    PID_calc(&_motor->pid_param_angle, _motor->feedback.all_ecd, _motor->target_angle);
...
void CAN2_RX1_IRQHandler(void)
{
	...
	switch(Rx2Message.StdId)
	{
		...
		case 0x203: get_motor_measure(&rabbit.pick_up.motor[0].feedback, Rx2Message);break;
		case 0x204: get_motor_measure(&rabbit.pick_up.motor[1].feedback, Rx2Message);break;
	}
