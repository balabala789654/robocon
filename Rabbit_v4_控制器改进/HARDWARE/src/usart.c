...
//	USART_ITConfig(USART6, USART_IT_IDLE, ENABLE);
	...
	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)p3_receive;
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
...
void USART6_IRQHandler(void)
{
	...
	{
		p3_receive[i] = USART_ReceiveData(USART6);
		i++;
		if(i==13) 
		{
			if(flag) 
			{
				head_verify(p3_receive);
				flag=0;
			}
			p3_data=p3_remote_output(verify_data_func(p3_receive));
			i=0;
		}
		USART_ClearFlag(USART6, USART_FLAG_RXNE);
	}
